...
local int
add_data_in_datablock (linkedlist_data *ll,
const void *buf,
uLong len)
{
  linkedlist_datablock_internal *ldi;
...
local int
write_datablock (FILE *fout, linkedlist_data *ll)
{
  linkedlist_datablock_internal *ldi;
  ldi = ll->first_block;
  while (ldi != NULL)
    ...
    int append)
{
  ...
  ziinit.filezip = fopen (pathname, (append == 0) ? "wb8" : "ab");
...
int level)
{
  ...
  if (zipfi == NULL)
	...
	zi->ci.dosDate =
	  ziplocal_TmzDateToDosDate ((tm_zip *)&zipfi->tmz_date, zipfi->dosDate);
    }
