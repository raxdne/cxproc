...
cmarkTreeToDOM(xmlNodePtr pndArgBlock, xmlNodePtr pndArg, cmark_node* pcmnArg)
{
  ...
  if (pcmnArg) {
	...
	if (StringBeginsWith((char *)pcmnArg->first_child->data, "http://")) {
	  pucDisplay = xmlStrdup(BAD_CAST &(pcmnArg->first_child->data[7]));
	  xmlSetProp(pndT, BAD_CAST "href", pcmnArg->as.link.url);
	}
	else if (StringBeginsWith((char *)pcmnArg->first_child->data, "https://")) {
	  pucDisplay = xmlStrdup(BAD_CAST &(pcmnArg->first_child->data[8]));
	  xmlSetProp(pndT, BAD_CAST "href", pcmnArg->as.link.url);
	}
	...
	if (STR_IS_NOT_EMPTY(pucDisplay)) {
	  ...
	  xmlAddChild(pndT, xmlNewText(pcmnArg->as.link.url));
	}
	xmlFree(pucDisplay);
      }
    }
