  ...
  if (STR_IS_NOT_EMPTY(pucArg)) {
    ...
    */
	...
	if ((pucT = pieElementGetBeginPtr(ppeT)) != NULL) {
	  xmlAddChild(pndBlock, xmlNewPI(BAD_CAST"tag-regexp", &pucT[6]));
	}
      }
      else {
...
SplitNodeToTableDataNodes(xmlNodePtr pndArgParent, xmlChar* pucPatternSep)
{
  ...
  if (pndArgParent == NULL) {
    ...
    for (pucBegin = pucSep = pndChild->content; (pucSep = BAD_CAST xmlStrstr(pucSep, pucPatternSep)); ) {
      ...
      if (fMix && StringEndsWithEntity(pucBegin, pucSep - pucBegin)) {
...
TranslateUncToUrl(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	pucUnc = xmlStrndup(pucA, ovector[1] - ovector[0]);
	...
	if (ovector[0] > 6 && xmlStrncasecmp(BAD_CAST &pucArg[ovector[0] - 7], BAD_CAST"file://", 7) == 0) {
	  ...
	  pucRelease = xmlStrndup(pucArg, ovector[0]);
	...
	if (pucArg[ovector[0]] == '\\' && pucArg[ovector[0] + 1] == '\\') {
	  /* UNC noatation */
	  pucRelease = xmlStrndup(&(pucArg[ovector[0] + 1]), ovector[1] - ovector[0] - 1);
	}
	else {
	  pucRelease = xmlStrndup(&(pucArg[ovector[0]]), ovector[1] - ovector[0]);
	}
...
GetLinkDisplayNameNew(const xmlChar *pucArg)
{
  ...
  if (pucArg) {
    ...
    if (b > 0 && l > 12) {
      pucResult = xmlStrndup(&pucArg[b], l);
...
SplitTupelToLinkNodes(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	pucPre = xmlStrndup(&pucArg[ovector[0]], ovector[i*2+1] - ovector[i*2]);
...
#if 1
	pucUrl = xmlStrndup(&pucArg[ovector[i*2]], ovector[i*2+1] - ovector[i*2]);
#else
	xmlChar *pucT;
...
SplitTupelToLinkNodesMd(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	pucPre = xmlStrndup(pucArg, ovector[0]);
...
SplitStringToLinkNodes(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	if (xmlStrcasestr(pucA, BAD_CAST"file://.") == pucA) {
	  ...
	  pucUrl = xmlStrndup(pucA + 7, ovector[1] - ovector[0] - 7);
	}
	else {
	  pucUrl = xmlStrndup(pucA, ovector[1] - ovector[0]);
	}
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
    ...
    if (rc > -1) {
	 ...
	 element node
      ...
      if (ovector[i*2+1] - ovector[i*2] > 0) {
	if (ovector[i*2] > 0) {
	  pucPre = xmlStrndup(pucArg, ovector[i*2]);
	...
	pucScript = xmlStrndup(&pucArg[ovector[i*2]], ovector[i*2+1] - ovector[i*2]);
...
SplitStringToInlineNodes(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	*/
	...
	for (i0=ovector[0]; pucArg[i0] == pucArg[ovector[0]]; i0++);
	for (i1=ovector[1]; i1 > 0 && pucArg[i1-1] == pucArg[ovector[0]]; i1--);
...
SplitStringToDateNodes(const xmlChar *pucArg)
{
  ...
  if (pucArg != NULL && (ducOrigin = xmlStrlen(pucArg)) > 0) {
	...
	*/
	...
	if (rc > 2) {
	  pucDate = xmlStrndup(pucA, ovector[5] - ovector[2]);
	}
	else {
	  pucDate = xmlStrndup(pucA, ovector[3] - ovector[2]);
	}
...
_WeightPatternInTree(xmlNodePtr pndArg, xmlChar *pucArgPattern)
{
  ...
  if (xmlNodeIsText(pndArg)) {
    iResult += GetNumberOfMatches(pndArg, pucArgPattern);
  }
#if 0
  else if (domNodeIsAttribute(pndArg)) {
    xmlChar *pucContent;
    xmlNodePtr pndProp;

    if ((pucContent = BAD_CAST pndArg->name) != NULL && xmlStrcasestr(pucContent, pucArgPattern) != NULL) {
      /* pattern found */
      iResult++;
    }

    if ((pucContent = domNodeGetContentPtr(pndArg)) != NULL && xmlStrcasestr(pucContent, pucArgPattern) != NULL) {
      /* pattern found */
      iResult++;
    }
  }
    ...
    if (domNodeIsDocRoot(pndArg)) {
      xmlSetProp(pndArg, BAD_CAST"pattern", pucArgPattern);
    }
#if 0
    else {
      xmlChar *pucContent;
	...
	iResult++;
      ...
      /*! compare attribute names and values with pattern */
      for (patT = pndArg->properties; patT != NULL; patT = patT->next) {
	if (xmlStrEqual(patT->name, pucArgPattern)) {
	  iResult++;
	}
	iResult += _WeightPatternInTree(patT->children, pucArgPattern);
      }
    }
#endif

    for (pndChild = pndArg->children; pndChild != NULL; pndChild = pndChild->next) {
      iResult += _WeightPatternInTree(pndChild, pucArgPattern);
    }

    if (iResult > 0) {
      domIncrProp(pndArg, BAD_CAST"w", iResult);
    }
  }
  else {
  }
  return iResult;
} /* End of _WeightPatternInTree() */
...
TaskNodeNew(xmlNodePtr pndArg)
{
  ...
  if (IS_NODE_PIE_PAR(pndArg) && 
	...
	pucT = xmlStrndup(pucContent, ovector[3]);
	...
	if ((pndChild = pndArg->children) != NULL
	  ...
	  if (rc > -1) {
	    ...
	    if (pcre2_get_ovector_count(match_data) > 1 && ovector[1] - ovector[0] > 3) {
	      ...
	      pucRelease = xmlStrndup(pucSubstr + ovector[4], ovector[5] - ovector[4]);
		...
		pucT = xmlStrndup(pucSubstr + ovector[6], ovector[7] - ovector[6]);
	      }
	      else {
