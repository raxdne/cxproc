...
cxpTest(cxpContextPtr pccArg)
{
  ...
  if (RUNTEST) {
    ...
    else if ((prnTT = resNodeListFindPath(cxpCtxtSearchGet(pccT), BAD_CAST"config.cxp")) == NULL) {
  ...
  if (RUNTEST) {
    ...
    else if ((prnTT = resNodeListFindPath(cxpCtxtSearchGet(pccTT), BAD_CAST"config.cxp")) == NULL) {
      printf("Error resNodeListFindPath()\n");
    }
    else if (resNodeIsFile(prnTT) == FALSE) {
      printf("Error resNodeIsFile()\n");
