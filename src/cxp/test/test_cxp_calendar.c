...
calTest(cxpContextPtr pccArg)
{
  ...
  if (RUNTEST) {
    ...
    else if (AddTreeYear(NULL,-1) != FALSE) {
      printf("Error AddTreeYear()\n");
    }
    else if (AddTreeYear(pCalendarT,2009) == FALSE) {
      printf("Error AddTreeYear()\n");
    }
    else {
      n_ok++;
  ...
  if (RUNTEST) {
    ...
    pndMakeCalendar = xmlNewNode(NULL, BAD_CAST NAME_CALENDAR);
    ...
    pndCol = xmlNewChild(pndMakeCalendar, NULL, BAD_CAST NAME_COL, NULL);
    xmlSetProp(pndCol, BAD_CAST"id", BAD_CAST"test");
    pndChild = xmlNewChild(pndCol, NULL, BAD_CAST NAME_XML, NULL);
    pndPie = xmlNewChild(pndChild, NULL, BAD_CAST NAME_PIE, NULL);
    pndPieImport = xmlNewChild(pndPie, NULL, BAD_CAST NAME_PIE_IMPORT, pucData);
    ...
    pndChild = xmlNewChild(pndCol, NULL, BAD_CAST NAME_XML, NULL);
    pndPie = xmlNewChild(pndChild, NULL, BAD_CAST NAME_PIE, NULL);
    pndPieImport = xmlNewChild(pndPie, NULL, BAD_CAST NAME_PIE_IMPORT, NULL);
  ...
  if (RUNTEST) {
    ...
    pndMake = xmlNewDocNode(pdocCalendarDefinition, NULL, BAD_CAST NAME_MAKE, NULL);
    ...
    pndChild = xmlNewChild(pndMake, NULL, BAD_CAST NAME_XML, NULL);
    xmlSetProp(pndChild, BAD_CAST "name", BAD_CAST TEMPPREFIX "calendar-1.xml");
    pndMakeCalendar = xmlNewChild(pndChild, NULL, BAD_CAST NAME_CALENDAR, NULL);
    //xmlSetProp(pndMakeCalendar, BAD_CAST "year", BAD_CAST"2015");
    ...
    pndChild = xmlNewChild(pndCol, NULL, BAD_CAST NAME_XML, NULL);
    pndChild = xmlNewChild(pndChild, NULL, BAD_CAST NAME_DIR, NULL);
    ...
    pndChild = xmlNewChild(pndCol, NULL, BAD_CAST NAME_XML, NULL);
    pndChild = xmlNewChild(pndChild, NULL, BAD_CAST NAME_FILE, NULL);
    ...
    pndChild = xmlNewChild(pndCol, NULL, BAD_CAST NAME_XML, NULL);
    pndChild = xmlNewChild(pndChild, NULL, BAD_CAST NAME_PIE, NULL);
    pndChild = xmlNewChild(pndChild, NULL, BAD_CAST NAME_PIE_IMPORT, NULL);
  ...
  if (SKIPTEST) {
    ...
    pndPie = xmlNewNode(NULL, BAD_CAST NAME_PIE);

    if ((pccT = cxpCtxtDup(pccArg)) == NULL) {
      printf("Error cxpCtxtDup()\n");
    }
    else if ((pndDir = xmlNewChild(pndPie, NULL, BAD_CAST NAME_DIR, NULL)) == NULL
      || xmlSetProp(pndDir, BAD_CAST"depth", BAD_CAST"1") == NULL
      || xmlSetProp(pndDir, BAD_CAST"verbosity", BAD_CAST"3") == NULL) {
      printf("Error xmlNewChild()\n");
    }
    else if ((pndT = xmlNewChild(pndDir, NULL, BAD_CAST NAME_DIR, NULL)) == NULL
      || xmlSetProp(pndT, BAD_CAST"name", BAD_CAST TESTPREFIX) == NULL) {
      printf("Error xmlNewChild()\n");
    }
    else if ((pdocDir = dirProcessDirNode(pndDir, NULL, pccT)) == NULL) {
      printf("Error dirProcessDirNode()\n");
    }
    else if ((pdocTest = calProcessCalendarNode(xmlDocGetRootElement(pdocDir), pccT)) == NULL) {
      printf("Error calProcessCalendarNode()\n");
    }
    else {
      n_ok++;
