...
cxpCtxtSaveFileNode(cxpContextPtr pccArg,xmlNodePtr pndArg,xmlDocPtr pdocArgOutput,xmlChar *pucArgOutput)
{
  ...
  if (prnOutput) { /* Save the result only if a \@name is given. */
    if (pdocArgOutput) {
      /* this is DOM content */
      iContentLength = cxpCtxtOutputSaveFormat(pccArg,prnOutput,pdocArgOutput,NULL,(char *)((pucAttrEncoding != NULL) ? pucAttrEncoding : "UTF-8"),1,FALSE);
    }
    else if (pucArgOutput) {
      /* this is string content only */
      iContentLength = cxpCtxtOutputSaveFormat(pccArg,prnOutput,NULL,pucArgOutput,(char *)((pucAttrEncoding != NULL) ? pucAttrEncoding : "UTF-8"),0,fAppend);
    }
  }
  else {
#ifdef DEBUG
